...
#added 2024 02 05
#soupx.X=soupx.X.round(decimals=0)
...
#print(subset_name)
ref_soupx=soupx[soupx.obs_names.isin(subset_name)].copy()
tmp2=anno[anno['malignant']=='malignant']
query_name=tmp2['NAME'].values.tolist()
#print(ref_name)
subset_soupx=soupx[soupx.obs_names.isin(query_name)].copy()
...
print("Ref Acc: {}".format(np.mean(reference_latent.obs.predictions == reference_latent.obs.Broad_cell_identity)))

sc.pp.neighbors(reference_latent, n_neighbors=8)
sc.tl.leiden(reference_latent)
sc.tl.umap(reference_latent)
sc.pl.umap(reference_latent,
           color=['predictions', 'Broad_cell_identity'],
           frameon=False,
           wspace=0.6,
...
print("Query Acc: {}".format(np.mean(query_latent.obs.predictions == query_latent.obs.Broad_cell_identity)))
subset_soupx.obs['Broad_cell_identity'] = subset_soupx.obs['orig_cell_types'].tolist()
...
full_latent.obs['predictions'] = model.predict(adata=soupx_full)
print("Full Acc: {}".format(np.mean(full_latent.obs.predictions == full_latent.obs.Broad_cell_identity)))
